...
import { ArrowLeft, UserPlus, Users, Search, ShieldCheck, UserCog } from "lucide-react";
...
export default function AdminUsersPage() {
    ...
    try {
      ...
      const requests = await Promise.all(
        roles.map(async (role) => {
          try {
            return await useCases.getAdminUsers.execute(role);
          } catch {
            return [] as User[];
          }
        })
      ...
      const merged = Array.from(new Map(requests.flat().map((u) => [u.id, u])).values());
      setUsers(merged);
    } catch {
      setUsers([]);
    } finally {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      await useCases.createUserAdmin.execute({
        nombre,
        apellido,
        correo,
        telefono,
                  ...
                  <tr key={u.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4">
                      {u.rol?.toLowerCase() === "administrador" && (
                        <span className="inline-flex items-center gap-1 rounded-full bg-sky-500/10 px-3 py-1 text-xs font-bold text-sky-600 border border-sky-400/20">
                          Administrador
                        </span>
                      )}
                      {u.rol?.toLowerCase() === "contador" && (
                        <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-bold text-emerald-600 border border-emerald-400/20">
                          Contador
                        </span>
                      )}
                      {u.rol?.toLowerCase() === "vendedor" && (
                        <span className="inline-flex items-center gap-1 rounded-full bg-purple-500/10 px-3 py-1 text-xs font-bold text-purple-600 border border-purple-400/20">
                          Vendedor
                        </span>
                      )}
                      {u.rol?.toLowerCase() === "cliente" && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h3 className="font-display text-2xl text-slate-900">Crear Nuevo Personal / Usuario</h3>
            ...
            <form onSubmit={handleCreateUser} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="mt-1 h-11 w-full rounded-xl border border-slate-200 px-4 text-sm outline-none focus:border-sky-500 font-bold text-slate-700"
                >
